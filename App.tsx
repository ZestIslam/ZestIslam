...
import { signInUser, signUpUser, signOutUser, resetUserPassword, sendContactMessage } from './services/userService';
...
const App: React.FC = () => {
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
      ...
      // Whether automatic DB save worked or not, we show success or trigger fallback
      if (success) {
          setSentSuccess(true);
          setContactForm({ name: '', email: '', message: '' });
          setTimeout(() => setSentSuccess(false), 5000);
      } else {
          // Fallback: If DB write fails, open email client automatically
          const subject = encodeURIComponent(`Contact from ${contactForm.name}`);
          const body = encodeURIComponent(`Name: ${contactForm.name}\nEmail: ${contactForm.email}\n\nMessage:\n${contactForm.message}`);
          window.location.href = `mailto:zestislam@gmail.com?subject=${subject}&body=${body}`;
          
          // Show success UI as well to confirm action taken
          setSentSuccess(true);
          setContactForm({ name: '', email: '', message: '' });
      }
  };
