    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-800 flex gap-3 text-sm text-indigo-800 dark:text-indigo-200">
                ...
                <p>True interpretation requires a qualified scholar. Good dreams are from Allah, bad ones are from Shaytan.</p>
            </div>
                    ...
                    <div className="p-6 space-y-6">
                        <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex">
                            {(['english', 'urdu', 'hinglish'] as Language[]).map((l) => (
                                <button key={l} onClick={() => setLang(l)} className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase transition-all ${lang === l ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-400'}`}>{l}</button>
                            ))}
                        </div>
                        <div className={`prose prose-sm dark:prose-invert text-slate-700 dark:text-slate-300 ${lang === 'urdu' ? 'text-right' : ''}`} dir={lang === 'urdu' ? 'rtl' : 'ltr'}>
                            <p className={`leading-relaxed ${lang === 'urdu' ? 'font-arabic text-lg' : ''}`}>
                                {result[lang]?.interpretation || 'Interpretation details unavailable.'}
                            </p>
                        </div>
                        {result[lang]?.symbols && result[lang]?.symbols.length > 0 && (
                            <div className={lang === 'urdu' ? 'text-right' : ''} dir={lang === 'urdu' ? 'rtl' : 'ltr'}>
                                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">{lang === 'urdu' ? 'علامات' : 'Key Symbols'}</h4>
                                <div className={`flex flex-wrap gap-2 ${lang === 'urdu' ? 'justify-end' : ''}`}>
                                    {result[lang]?.symbols.map((s, i) => (
                                        <span key={i} className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-full text-sm font-medium">{s}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                        <div className={`bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-xl border border-emerald-100 dark:border-emerald-800 ${lang === 'urdu' ? 'text-right' : ''}`} dir={lang === 'urdu' ? 'rtl' : 'ltr'}>
                            <h4 className={`text-sm font-bold text-emerald-800 dark:text-emerald-300 mb-2 flex items-center gap-2 ${lang === 'urdu' ? 'flex-row-reverse' : ''}`}><BookOpen className="w-4 h-4" /> {lang === 'urdu' ? 'روحانی نصیحت' : 'Spiritual Advice'}</h4>
                            <p className={`text-sm text-emerald-700 dark:text-emerald-400 italic ${lang === 'urdu' ? 'font-arabic text-lg not-italic' : ''}`}>"{result[lang]?.advice || 'Seek refuge in Allah.'}"</p>
                        </div>
                    </div>
                </div>
            )}
