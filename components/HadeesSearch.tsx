...
import { Search, BookOpen, Loader2, Sparkles, Volume2, Book } from 'lucide-react';
  ...
  const renderExpandedSharh = (expanded: {idx: number, data: SharhResult, lang: 'english' | 'urdu' | 'hinglish'}) => (
        <div className="mt-6 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-fade-in-up">
            <div className="p-6 md:p-8">
                ...
                <p 
                    className={`text-slate-700 dark:text-slate-300 mb-8 leading-relaxed font-medium ${expanded.lang === 'urdu' ? 'font-quran text-2xl text-right' : 'text-base'}`}
                    dir={expanded.lang === 'urdu' ? 'rtl' : 'ltr'}
                >
                    {expanded.data[expanded.lang].paragraph}
                </p>
                
                <div>
                    <h5 className={`font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wide mb-4 flex items-center gap-2 ${expanded.lang === 'urdu' ? 'flex-row-reverse' : ''}`}>
                        <Sparkles className="w-3 h-3 text-emerald-500" />
                        {expanded.lang === 'urdu' ? 'اہم نکات' : 'Key Benefits'}
                    </h5>
                    <ul 
                        className={`space-y-3 ${expanded.lang === 'urdu' ? 'text-right' : ''}`}
                        dir={expanded.lang === 'urdu' ? 'rtl' : 'ltr'}
                    >
                        {expanded.data[expanded.lang].points.map((p, i) => (
                            <li key={i} className={`flex gap-4 items-start p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm ${expanded.lang === 'urdu' ? 'flex-row-reverse' : ''}`}>
                                ...
                                <span className={`text-slate-700 dark:text-slate-300 ${expanded.lang === 'urdu' ? 'font-quran text-xl' : 'text-sm'}`}>{p}</span>
                            </li>
                        ))}
                    </ul>
                </div>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <div className="text-center space-y-4 py-8">
        <h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Hadith Search</h2>
        <p className="text-slate-500 dark:text-slate-400 max-w-lg mx-auto leading-relaxed">Search authentic traditions and gain insights.</p>
      </div>
        ...
        <>
            <form onSubmit={handleSearch} className="relative group max-w-2xl mx-auto animate-fade-in z-10">
                ...
                <input
                    ...
                    placeholder="Try 'Rights of neighbors', 'Patience'..."
            ...
            <div className="space-y-6">
                ...
                {/* Initial State */}
                {!loading && !searched && (
                    <div className="text-center py-16 opacity-50 max-w-lg mx-auto">
                        <Book className="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
                        <p className="text-slate-500 dark:text-slate-400">Search for any topic to retrieve authentic Hadiths from Sahih Bukhari, Muslim, and other major collections.</p>
                    </div>
                )}
                    ...
                    <div key={idx} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300 relative group overflow-hidden animate-fade-in-up">
                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => playingAudioId === hadith.arabicText ? stopAudio() : playAudio(hadith.arabicText)} className={`p-2 rounded-full transition-colors ${playingAudioId === hadith.arabicText ? 'text-red-500 bg-red-50 dark:bg-red-900/20' : 'text-slate-300 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/30'}`} title="Play Audio">
                                    {playingAudioId === hadith.arabicText ? <div className="w-3 h-3 bg-current rounded-sm" /> : <Volume2 className="w-5 h-5" />}
                                </button>
                            </div>
                        </div>
