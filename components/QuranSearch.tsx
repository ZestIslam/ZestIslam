  ...
  if (readingSurah) {
      return (
          <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-32">
              <div className="sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-20 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between px-2">
                  <button onClick={() => setReadingSurah(null)} className="flex items-center gap-2 text-slate-500 hover:text-emerald-600 transition-colors"><ChevronLeft className="w-5 h-5" /> Back</button>
                  ...
                  <div className="flex items-center gap-2">
                      <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex items-center gap-2 px-3">
                            ...
                            <select value={playbackSpeed} onChange={(e) => setPlaybackSpeed(parseFloat(e.target.value))} className="bg-transparent text-xs font-bold text-slate-600 dark:text-slate-300 border-none focus:ring-0 cursor-pointer py-1 pr-2">
                                <option value="0.75">0.75x</option><option value="1">1.0x</option><option value="1.25">1.25x</option>
                            </select>
                      </div>
                      <button onClick={() => handleListenFullSurah(readingSurah.meta.number)} className="bg-emerald-600 hover:bg-emerald-700 text-white p-2 rounded-lg transition-colors">
                         {audioLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                      </button>
                  </div>
              </div>
              <div className="text-center py-8 font-quran text-3xl text-slate-800 dark:text-emerald-400 opacity-80">بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ</div>
                  ...
                  {readingSurah.verses.map((verse, idx) => (
                      <div key={verse.number} className={`bg-white dark:bg-slate-900 p-6 rounded-[2rem] border transition-all group ${currentTrackIndex === idx ? 'border-emerald-500 ring-4 ring-emerald-500/10' : 'border-slate-100 dark:border-slate-800 hover:border-emerald-100 dark:hover:border-emerald-900'}`}>
                          <div className="flex justify-between items-start mb-6">
                              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border ${currentTrackIndex === idx ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-700'}`}>{verse.numberInSurah}</span>
                              <button onClick={() => playAudio(verse.text)} className={`p-2 rounded-full transition-all ${playingAudioId === verse.text ? 'bg-red-50 dark:bg-red-900/20 text-red-500' : 'text-slate-300 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30'}`}>
                                  {playingAudioId === verse.text ? <Square className="w-5 h-5 fill-current" /> : <Volume2 className="w-5 h-5" />}
                              </button>
                          </div>
                          <p className="text-right font-quran text-3xl md:text-4xl text-slate-800 dark:text-white mb-6 leading-[2.5]" dir="rtl">{verse.text}</p>
                          <p className="text-slate-600 dark:text-slate-300 text-lg font-serif leading-relaxed">{verse.translation}</p>
                      </div>
                  ))}
              </div>
              <div className="text-center pt-8"><button onClick={() => setReadingSurah(null)} className="px-8 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-full font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Close Reader</button></div>
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 p-4 z-50 animate-fade-in-up">
                    <div className="max-w-4xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div><p className="text-sm font-bold text-slate-800 dark:text-white">Playing Surah {readingSurah.meta.englishName}</p><p className="text-xs text-slate-500">Ayah {currentTrackIndex + 1} of {readingSurah.meta.numberOfAyahs}</p></div>
                        </div>
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={togglePlayPause} className="w-10 h-10 bg-emerald-600 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-105 transition-transform">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="text-center space-y-4 py-8">
        <h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Quranic AI Guidance</h2>
        <p className="text-slate-500 dark:text-slate-400 max-w-lg mx-auto leading-relaxed">Search verses by topic or emotion, or read full Surahs with AI insights.</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="relative group max-w-2xl mx-auto">
        ...
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search topic (e.g., 'Patience') or Surah name..." className="w-full pl-16 pr-32 py-5 rounded-full bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 shadow-lg shadow-slate-200/50 dark:shadow-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all text-lg text-slate-700 dark:text-slate-200 placeholder:text-slate-400" />
        <div className="absolute inset-y-2 right-2">
            <button type="submit" disabled={loading || !query} className="h-full bg-slate-900 dark:bg-emerald-600 hover:bg-emerald-600 text-white px-8 rounded-full font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
      ...
      {matchedSurah && !searched && (
          <div className="max-w-2xl mx-auto animate-fade-in-up">
              <div onClick={() => openFullSurah(matchedSurah.number)} className="bg-emerald-600 text-white p-6 rounded-[2rem] flex items-center justify-between cursor-pointer hover:bg-emerald-700 transition-colors shadow-xl shadow-emerald-200 dark:shadow-none group">
                  <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center font-bold text-xl font-quran">{matchedSurah.number}</div>
                      <div><h3 className="text-xl font-bold">Surah {matchedSurah.englishName}</h3><p className="text-emerald-100 text-sm">{matchedSurah.englishNameTranslation} • {matchedSurah.numberOfAyahs} Ayahs</p></div>
                  </div>
                  <div className="w-10 h-10 bg-white text-emerald-600 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">{readerLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <BookOpen className="w-5 h-5" />}</div>
              </div>
          </div>
      )}
      ...
      <div className="space-y-6">
        {results.map((verse, idx) => (
          <div key={idx} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300 relative group">
              <div className="flex justify-between items-start mb-6">
                  <span className="inline-flex items-center px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm font-bold border border-slate-100 dark:border-slate-700"><BookOpen className="w-4 h-4 mr-2 text-emerald-500" />{verse.surahName} : {verse.verseNumber}</span>
              ...
              <div className="bg-slate-50/50 dark:bg-slate-800/30 rounded-2xl p-6 mb-6 border border-slate-50 dark:border-slate-800"><p className="text-right font-quran text-2xl md:text-3xl text-slate-800 dark:text-white" dir="rtl">{verse.arabicText}</p></div>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed font-serif text-lg mb-6">{verse.translation}</p>
              <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-6">
                 <p className="text-xs text-slate-400 italic max-w-lg">{verse.explanation}</p>
                 <button onClick={() => handleTadabbur(idx, verse)} className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold text-sm hover:bg-emerald-50 dark:hover:bg-emerald-900/30 px-4 py-2 rounded-xl transition-colors">{tadabburLoading === idx ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}Reflect</button>
              </div>
                  ...
                  <div className="mt-6 bg-emerald-50/50 dark:bg-emerald-900/10 p-6 rounded-2xl animate-fade-in-up">
                      <div className="flex gap-2 mb-4">
                          {['english', 'urdu', 'hinglish'].map(l => (
                              <button key={l} onClick={() => setExpandedTadabbur(prev => prev ? {...prev, lang: l as any} : null)} className={`text-xs font-bold uppercase px-3 py-1 rounded-lg ${expandedTadabbur.lang === l ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-500'}`}>{l}</button>
                          ))}
                      </div>
                      <p className={`text-slate-700 dark:text-slate-300 mb-4 ${expandedTadabbur.lang === 'urdu' ? 'text-right font-quran text-lg' : ''}`}>{expandedTadabbur.data[expandedTadabbur.lang].paragraph}</p>
                      <ul className={`space-y-2 ${expandedTadabbur.lang === 'urdu' ? 'text-right' : ''}`}>
                          {expandedTadabbur.data[expandedTadabbur.lang].points.map((p, i) => (
                              <li key={i} className="text-sm text-slate-600 dark:text-slate-400 flex gap-2"><span className="text-emerald-500">•</span> {p}</li>
                          ))}
                      </ul>
                  </div>
              )}
                ...
                {allSurahs.map(surah => (
                    <button key={surah.number} onClick={() => openFullSurah(surah.number)} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 text-left transition-all group hover:shadow-md">
                        <div className="flex items-center justify-between mb-2">
                            <span className="w-8 h-8 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-xs font-bold text-slate-400 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900 group-hover:text-emerald-600 transition-colors">{surah.number}</span>
